...
DB_FILEPATH = os.path.join(os.path.dirname(
    __file__), '/home/jack/Desktop/northwind_small.sqlite3')
...
# Use SQL julianday function:
# https://www.w3resource.com/sqlite/sqlite-julianday.php
query = """
SELECT AVG(julianday(HireDate) - julianday(BirthDate)) /
365 AS "Average hire age"
FROM Employee
"""
result = curs.execute(query).fetchall()
...
query = """
SELECT City, AVG(julianday(HireDate) - julianday(BirthDate)) /
365 AS "Average hire age"
FROM Employee
GROUP BY City
"""
result = curs.execute(query).fetchall()
print('P2Q3 How avg age of employee at hire vary by city: ', result, '\n')


print("--- Sprint Part 3 Questions ---\n")

# P3Q1: What are the ten most expensive items in database & their suppliers?
query = """
...
"""
result = curs.execute(query).fetchall()
print('P3Q1 Top 10 most expensive products AND their supplier: ', result, '\n')
